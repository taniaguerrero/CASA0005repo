...
22. Here load the gis_osm_pois_a_free_1 polygon.shp layer.
...
```{r echo=FALSE, fig.align='center', cache=TRUE}
knitr::include_graphics('prac5_data/QGIS_model.png') 
```
...
Your R and geogprahical are certainly improving by now, so i'm just going to provide you with the R code i used to do this analysis..we'll then have a quick look at making a Shiny app using the data --- which i'll explain fully.
...
Londonborough <- read_shape("Prac1_data/statistical-gis-boundaries-london/ESRI/London_Borough_Excluding_MHW.shp", as.sf = TRUE)

Airbnb <- read_csv("prac5_data/listings.csv")
  
Worldcities <- read_shape("prac5_data/World_Cities/World_Cities.shp", as.sf = TRUE)

# load as sp to use with spatialpointsdataframe  
UK_outline <- read_shape("prac5_data/gadm36_GBR_shp/gadm36_GBR_0.shp", as.sf = TRUE)
...
st_crs(Londonborough) <- 27700
st_crs(OSM) <- 27700


t=st_within(OSM, Londonborough)

pointinpoly <- st_join(OSM, Londonborough, join = st_within)

t=as.data.frame(pointinpoly)


tree_tract_count <- count(pointinpoly$osm_id, pointinpoly$GSS_CODE) %>%
  print()

res <- over(OSM, Londonborough)


t=aggregate(OSM["osm_id"], Londonborough, count)
