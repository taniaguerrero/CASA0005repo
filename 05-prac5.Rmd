...
In this practical we're going to focus on creating mapped outputs using QGIS, ArcMap and R. For fun we're going to use data from OpenStreetMap (OSM) and Airbnb. OpenStreetMap is a  collaborative project that has created a free editable map of the World. Airbnb is an online marketplace that connects people looking to rent homes to those seeking accomodation often over short time periods.
...
OSM <- read_shape("prac5_data/greater-london-latest-free.shp/gis_osm_pois_a_free_1.shp",
                  as.sf = TRUE)
...
legend <- tm_shape(Hotels) +
    ...
    tm_scale_bar(position=c(0.2,0.04), size=0.6)+
...
pal <- colorBin(palette = "YlOrRd", domain=t$freq, bins=breaks)
pal2 <- colorBin(palette = "YlOrRd", domain=t$`Accom count`, bins=breaks)
