...
On Twitter and wan't to see the love for shapefiles....have a look at [the shapefile account](https://twitter.com/shapefiIe)
...
**Make sure** you read the instructions provided by the UCL Software Database fully if installing onto your own machine as you have to set the license correctly 
...
Load the ```sf``` package so we can read our shapefile in --- remember to change to filepath to your shapefile. For this first practical we'll just leave the datea where it is, next week we'll show you how to use projects to make calling data much easier.
...
Now, let's make a quick thematic map (or a qtm) using the package  ```tmap ```. I've made mine for flytipping between 2011 and 2012 (column 2011_12): 
