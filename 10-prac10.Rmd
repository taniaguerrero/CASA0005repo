...
```{r echo=FALSE, out.width = "300", fig.align='center', cache=TRUE}
knitr::include_graphics('prac10_images/least_squares.png') 
```
...
29. Let's see an exmaple to show that if $\lambda$ is 0, then we have our box standard linear regression...as the plus value of $\lambda$ * the slope squared will equal 0...
...
31. Now we will find the best value for $\lambda$ using cross validation --- the lowest point of the curve is the optinmal value that best minimsed the error
