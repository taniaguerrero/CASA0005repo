...
Where $\beta_{1}$ is the intercept (expected value of Y when X=0, also called bias in machine learning), $\beta_{2}$ is the slope of the line and $\epsilon$ is the error term, the part that the model is unable to explain (and variables are not included)...
...
Remeber the formula produced from the regression equation, well we can also extract this automatically ...to make that original formula in RMarkdown I used this (not within a code chunk)....
