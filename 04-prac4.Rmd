...
12. Type some stuff (anything so that’s it’s not a blank empty file) at the top of the file and save it. Make sure your file is saved in your current project --- mine is as i can see it in the Files window (bottom right). If not, go File > Save As ...into your current project

You should see the files also appear under the Git tab

```{r echo=FALSE, out.width = "800pt", fig.align='center', cache=TRUE}
knitr::include_graphics('prac4_images/makeafile.png') 
```

13. As well as saving, which saves a copy to our local directory, we will also ‘commit’ or create a save point for our work on git. 
...
There is an easier way to set up Git and GitHub with your project, but this assumes you are starting fresh (with no code in an R project)!
...
Now you will be able to use Git and GitHub as per the previous instructions
